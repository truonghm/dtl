...
class MovieCrawler(BaseCrawler):
    ...
    def extract_movie_info(self, c, label_http, url_http):
        if label_http:
            ...
            if len(all_keys) == 0:
                ...
                if not (
                    self.movie[all_keys[0]] is None or len(self.movie[all_keys[0]]) == 0
                ):
                    return
                ...
                check = any(
                    [
                        self.movie[ak] is None or len(self.movie[ak]) == 0
                        for ak in all_keys
                    ]
                )
                if not check:
                ...
                if len(http) != 0:
                    ...
                    for content in inner_content:
                        ...
                        if key is not None and (
                            self.movie[key] is None or len(self.movie[key]) == 0
                        ):
                            self.movie[key] = value
                else:
    ...
    def crawl(self):
        ...
        self.movie["top_cast"] = [
            actor.text for actor in self.soup.select("a.sc-11eed019-1.jFeBIw")
        ]
        ...
        http2 = self.soup.select(
            "li.ipc-metadata-list__item.ipc-metadata-list-item--link"
        )
        ...
        return pd.DataFrame({k: [v] for k, v in self.movie.items()})
    ...
    def extract_revenue_usa_opening(text):
        ...
        full_patterns = f"(.*)(?={patterns})"
        new_text = re.sub(
            "[\$,]", "", re.sub(re.sub(full_patterns, "", text), "", text)
        )
    ...
    def __init__(self, load_from_cache: bool = True, stop_at: int = None):
        ...
        def do_load_from_cache(download_first: bool = True):
            if download_first:
                ...
                res = crawler.crawl(
                    write_to_cache=True, file_name=Setting.MOVIE_LIST_CACHE
                )

            cache = pd.read_csv(Setting.CACHE + Setting.MOVIE_LIST_CACHE + ".csv")
            self.urls = list(cache["urls"])
            self.movie_names = list(cache["movie_names"])
            self.movie_rating_counts = list(cache["movie_rating_counts"])
...
class MovieListCrawler(BaseCrawler):
    ...
    def crawl(self, write_to_cache: bool = True, file_name: str = None):
        res = dict(
            urls=[
                ...
                for element in self.soup.select("td.titleColumn a")
            ],
            movie_names=[
                re.findall("(?<=>)(.*?)(?=<)", str(t))[0]
                for t in self.soup.select("td.titleColumn")
            ],
            movie_rating_counts=[
                element.attrs.get("data-value")
                for element in self.soup.select("td.posterColumn span[name=nv]")
            ],
        )
